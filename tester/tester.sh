...
C_OPTIONS="-std=c++0x -fno-asm -Dasm=error -lm -O2 "
#
...
if [ "$EXT" = "java" ]; then
	...
	if [ $EXITCODE -ne 0 ]; then
		...
		echo 'Compile Error' >>$PROBLEMPATH/$UN/result.html
...
if [ "$EXT" = "py2" ]; then
	...
	if [ $EXITCODE -ne 0 ]; then
		...
		shj_log "$(cat cerr | head -10)"
		echo 'Syntax Error' >>$PROBLEMPATH/$UN/result.html
		(cat cerr | head -10 | sed 's/&/\&amp;/g' | sed 's/</\&lt;/g' | sed 's/>/\&gt;/g' | sed 's/"/\&quot;/g') >> $PROBLEMPATH/$UN/result.html
		cd ..
		rm -r $JAIL >/dev/null 2>/dev/null
...
if [ "$EXT" = "c" ] || [ "$EXT" = "cpp" ]; then
	...
	if [ $EXITCODE -ne 0 ]; then
		shj_log "Compile Error"
		shj_log "$(cat cerr | head -10)"
		echo 'Compile Error Error Messages: (line numbers are not correct)' >>$PROBLEMPATH/$UN/result.html
		# SHIELD_ACT=false
		# C_SHIELD_ON=true
		# if $C_SHIELD_ON; then
			# while read line; do
...
for((i=1;i<=TST;i++)); do
	...
	if [ "$EXT" = "java" ]; then
		if $PERL_EXISTS; then
			./runcode.sh $EXT $MEMLIMIT $TIMELIMIT $TIMELIMITINT $i "./timeout --just-kill -nosandbox -l $OUTLIMIT -t $TIMELIMIT java -mx${MEMLIMIT}k $JAVA_POLICY $MAINFILENAME"
		else
			./runcode.sh $EXT $MEMLIMIT $TIMELIMIT $TIMELIMITINT $i "java -mx${MEMLIMIT}k $JAVA_POLICY $MAINFILENAME"
		fi
		EXITCODE=$?
		if grep -iq -m 1 "Too small initial heap" out || grep -q -m 1 "java.lang.OutOfMemoryError" err; then
			shj_log "Memory Limit Exceeded"
			echo "Memory Limit Exceeded" >>$PROBLEMPATH/$UN/result.html
			continue
		fi
		if grep -q -m 1 "Exception in" err; then # show Exception
			...
			if $DISPLAY_JAVA_EXCEPTION_ON && grep -q -m 1 "^$javaexceptionname\$" ../java_exceptions_list; then
				echo "Runtime Error ($javaexceptionname)" >>$PROBLEMPATH/$UN/result.html
			else
				echo "Runtime Error" >>$PROBLEMPATH/$UN/result.html
			fi
			continue
		fi
		...
		if $SANDBOX_ON; then
			...
			if $PERL_EXISTS; then
				./runcode.sh $EXT $MEMLIMIT $TIMELIMIT $TIMELIMITINT $i "./timeout --just-kill --sandbox -l $OUTLIMIT -t $TIMELIMIT -m $MEMLIMIT ./$EXEFILE"
			else
				./runcode.sh $EXT $MEMLIMIT $TIMELIMIT $TIMELIMITINT $i "LD_PRELOAD=./EasySandbox.so ./$EXEFILE"
			fi
			EXITCODE=$?
			...
			if $PERL_EXISTS; then
				./runcode.sh $EXT $MEMLIMIT $TIMELIMIT $TIMELIMITINT $i "./timeout --just-kill -nosandbox -l $OUTLIMIT -t $TIMELIMIT -m $MEMLIMIT ./$EXEFILE"
			else
				./runcode.sh $EXT $MEMLIMIT $TIMELIMIT $TIMELIMITINT $i "./$EXEFILE"
			fi
			EXITCODE=$?
		fi
		...
		if $PERL_EXISTS; then
			./runcode.sh $EXT $MEMLIMIT $TIMELIMIT $TIMELIMITINT $i "./timeout --just-kill -nosandbox -l $OUTLIMIT -t $TIMELIMIT -m $MEMLIMIT python2 -O $FILENAME.py"
		else
			./runcode.sh $EXT $MEMLIMIT $TIMELIMIT $TIMELIMITINT $i "python2 -O $FILENAME.py"
		fi
		EXITCODE=$?

	elif [ "$EXT" = "py3" ]; then
		if $PERL_EXISTS; then
			./runcode.sh $EXT $MEMLIMIT $TIMELIMIT $TIMELIMITINT $i "./timeout --just-kill -nosandbox -l $OUTLIMIT -t $TIMELIMIT -m $MEMLIMIT python3 -O $FILENAME.py"
		else
			./runcode.sh $EXT $MEMLIMIT $TIMELIMIT $TIMELIMITINT $i "python3 -O $FILENAME.py"
		fi
		EXITCODE=$?
	...
	if ! grep -q "FINISHED" err; then
		if grep -q "SHJ_TIME" err; then
			...
			echo "Time Limit Exceeded" >>$PROBLEMPATH/$UN/result.html
			continue
			...
			shj_log "Hang Up"
			echo "Process hanged up" >>$PROBLEMPATH/$UN/result.html
			continue
		elif grep -q "SHJ_SIGNAL" err; then
			shj_log "Killed by a signal"
			echo "Killed by a signal" >>$PROBLEMPATH/$UN/result.html
			continue
		elif grep -q "SHJ_OUTSIZE" err; then
			shj_log "Output Size Limit Exceeded"
			echo "Output Size Limit Exceeded" >>$PROBLEMPATH/$UN/result.html
			continue
		fi
	else
	...
	if [ $EXITCODE -eq 137 ]; then
		...
		shj_log "Killed"
		echo "Killed" >>$PROBLEMPATH/$UN/result.html
		continue
	fi
	...
	value=$(tail -1 out)
	console_output=$(head -n -1 out)
	echo $console_output >>$PROBLEMPATH/$UN/result.html
